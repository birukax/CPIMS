<div class="">
    <button type="button"
        ...
        data-te-toggle="modal" data-te-target="#exampleModalCenter{{ $crime->status->id }}" data-te-ripple-init
...
<div data-te-modal-init
    ...
    id="exampleModalCenter{{ $crime->status->id }}" tabindex="-1"
    aria-labelledby="exampleModalCenter{{ $crime->status->id }}Title" aria-modal="true" role="dialog">
            ...
            @if ($crime->status->id === 1 && auth()->user()->role_id === 3)
                ...
                <div class="relative p-4">
                    <div
                        ...
                        <form method="post" action="/crimes/crime_detail/co_decision/{{ $crime->id }}" >
                            @method('PUT')
                            @csrf
                            <!--Message textarea-->
                            ...
                            <div class="justify-around flex gap-2">
                                <!--Submit button-->
                            <button type="submit"
                ...
                @elseif ($crime->status->id === 2 && auth()->user()->role_id === 5)
                <!--Modal body-->
                <div class="relative p-4">
                    <div
                        class="block max-w-md rounded-lg bg-white p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
                        <form method="post" action="/crimes/crime_detail/dc_decision/{{ $crime->id }}" >
                            @method('PUT')
                            @csrf
                            <!--Message textarea-->
                            <div class="relative mb-6" data-te-input-wrapper-init>
                                <textarea
                                    class="peer block min-h-[auto] w-full rounded border-0 bg-transparent px-3 py-[0.32rem] leading-[1.6] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none dark:text-neutral-200 dark:placeholder:text-neutral-200 [&:not([data-te-input-placeholder-active])]:placeholder:opacity-0"
                                    id="reviewTextArea" rows="5" name="dc_decision" placeholder="Message"></textarea>
                                <label for="reviewTextArea"
                                    class="pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] leading-[1.6] text-neutral-500 transition-all duration-200 ease-out peer-focus:-translate-y-[0.9rem] peer-focus:scale-[0.8] peer-focus:text-primary peer-data-[te-input-state-active]:-translate-y-[0.9rem] peer-data-[te-input-state-active]:scale-[0.8] motion-reduce:transition-none dark:text-neutral-200 dark:peer-focus:text-primary">review
                                </label>
                            </div>
